    ...
    if data['status'] == 'OK' and distances['status'] == 'OK':
        ...
        for i, result in enumerate(results):
            ...
            if 'Columbus' in result['formatted_address'] and 'GA' in result['formatted_address'] and query in name.lower() and distance <= radius:
                ...
                # Retrieve the closest street intersections using the Geocoding API
                ...
                if geocode_data['status'] == 'OK' and len(geocode_data['results']) > 0:
                    ...
                    street_intersections = [component['long_name'] for component in address_components if 'intersection' in component['types']]
                    
                    if street_intersections:
                        closest_intersection = street_intersections[0]
                    else:
                        closest_intersection = 'N/A'
                else:
                    closest_intersection = 'N/A'
                
                result_details.append((name, lat, lng, distance, city, state, closest_intersection))
...
if result_details:
    ...
    for i, details in enumerate(result_details, start=1):
        name, lat, lng, distance, city, state, intersection = details
        print(f"Result {i}:")
        print("Name:", name)
        print("City:", city)
        print("State:", state)
